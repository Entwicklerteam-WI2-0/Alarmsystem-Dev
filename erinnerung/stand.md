...
- **Stack-Update (#20):** **MySQL/MariaDB** als verpflichtende Backend-DB gesetzt — **ersetzt SQLite** im
  bisherigen T0 (FastAPI/HTTP bleiben). Betrifft Datenmodell, requirements und CI; DB-Treiber nötig.
...
- Noch **kein `src/`** — Scaffolding (P0 / Epic E-01 / DTB-1) ist überfällig.

## Als Nächstes (kritischer Pfad)
1. **P0/Scaffolding** (DTB-1: DTB-2/50/51/52) — `src/`-Struktur + erste Tests → macht CI grün-fähig.
