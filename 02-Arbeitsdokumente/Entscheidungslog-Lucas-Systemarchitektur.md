...
**E-40 — Fail-safe als Multi-Layer-Architektur (NF-01): jede Schicht erzwingt eigenständig `unknown`/nie GRÜN, inkl. DB-Ausfall**
- *Kontext/Task:* DTB-48 (ADR) · NF-01 (Fail-safe-Pflicht: bei veralteten/fehlenden/defekten Daten → `RiskLevel=unknown`, nie `green`) · E-34 (priorisierte Kaskade) · E-35 (DB-Ausfall-Schicht ist PyMySQL-spezifisch) · Backend-Konzept §6a(4). *Auslöser:* Der Fail-safe ist im Code bereits über mehrere Schichten verteilt umgesetzt, war aber **nicht als eine Architektur-Entscheidung dokumentiert** (DTB-48). **ID-Hinweis:** DTB-48 sah „E-39" vor (damals höchste = E-38); E-39 wurde inzwischen für das Audit-Log (DTB-29) vergeben → dieser ADR erhält die nächste freie ID **E-40** (Kollision analog E-29/E-32 vermieden).
- *Entscheidung:* NF-01 wird **nicht an einer einzelnen Stelle**, sondern als **mehrschichtige Fail-safe-Architektur** garantiert. Jede Schicht, die ihre Eingangs-Invariante nicht sicherstellen kann, erzwingt selbst `unknown` (nie GRÜN); **GRÜN nur, wenn ALLE Schichten ok sind**. Die Schichten:
  1. **Ingest/Aktualität (Stale):** `is_stale` (`failsafe.py`, DTB-13) — Reading älter als `stale_timeout_s` (Config) oder fehlend → `build_unknown_assessment` → `unknown`.
  2. **Sensor-Status (Fault):** `reading.status = fault` → `unknown` (AssessmentService, DTB-64).
  3. **Datenqualität/Plausibilität:** `check_plausibility` (Sprung/Flatline/Zeitstempelfehler, `Schwellenwerte.md §3`) → `unknown`.
  4. **Storage/DB-Ausfall:** `Repository.get_latest()`/`save()` brechen PyMySQL-Fehler (`OperationalError`/Verbindungsabbruch/Config) zu `RepositoryError` herunter → AssessmentService/Scheduler fail-safe; der Lese-Endpoint (DTB-43) liefert **HTTP 503** `Error{code,message}` — nie GRÜN, nie stiller Ausfall.
  5. **Assessment-Kaskade:** ist `ΔT` nicht berechenbar (`T_d` fehlt, `RH`/`T_a` defekt) → **Feuchte = wahr** (konservativ) ⇒ bei `T_s ≤ 0` mind. ORANGE, sonst GELB, nie GRÜN (**E-34**).
  6. **Serve-Zeit-Re-Check:** `build_assessment_current` (DTB-43/DTB-64) prüft Stale **und** Fault **erneut zum Abfragezeitpunkt** — eine zur Bewertung frische, beim Abruf veraltete Bewertung wird nicht als GRÜN ausgeliefert.
- *Begründung:* NF-01 verlangt „im Zweifel nie GRÜN" als **Default-Verhalten, nicht Sonderfall**. Eine einzige Prüfstelle ist fragil — sie deckt nicht ab, dass die Bewertung gut sein kann, während die DB weg ist, oder dass ein gespeicherter Wert zwischen Bewertung und Abruf altert. Defense-in-depth: jede Schicht hat ihren eigenen Fail-safe-Pfad, der sichere Zustand ist strukturell der Default. **Erreichbarkeit (DB-Fehler/Health) wird getrennt von Datenaktualität (Stale) behandelt** — zwei unabhängige `unknown`-Pfade (E-31: `/health` getrennt von `measured_at`). Der DB-Ausfall ist als **503 + `unknown`** modelliert (nicht als 500/Crash): ein Crash ist kein sicherer Zustand, ein expliziter 503 ist es.
- *Alternativen (verworfen):*
  - **Fail-safe nur in der Bewertungslogik (E-34 allein):** verworfen — deckt DB-Ausfall und das Altern zur Serve-Zeit nicht ab; NF-01 wäre lückenhaft.
  - **DB-Ausfall als 500/unbehandelte Exception:** verworfen — ein Crash/500 ist kein definierter sicherer Zustand; 503 + `unknown` ist explizit und fail-safe (E-35).
  - **Stale nur zur Bewertungszeit prüfen:** verworfen — die gespeicherte Bewertung altert; ohne Serve-Zeit-Re-Check könnte ein veralteter GRÜN-Wert ausgeliefert werden.
  - **GELB statt `unknown` als sicherer Zustand:** verworfen — GELB ist eine reguläre Bewertungsstufe der Kaskade (E-34); der Fail-safe braucht einen **eigenen** Zustand (`unknown`), sonst ist „echte Bewertung" nicht von „keine verlässliche Bewertung" unterscheidbar.
- *Ergebnis/Status:* dokumentiert (DTB-48). Code-Schichten implementiert: `src/assessment/failsafe.py` (Stale/Plausibilität/`build_unknown_assessment`), `src/storage/repository.py` + `assessment_repository.py` (`RepositoryError`), `src/assessment/service.py` (Assess-Zeit-NF-01), `src/main.py` (DTB-43 503-Pfad, Serve-Zeit-Re-Check). DTB-48 Jira auf E-40 korrigiert.
- *Bezug:* **NF-01** (Pflicht); **E-34** (Assessment-Schicht/Kaskade); **E-35** (DB-Ausfall-Schicht, rohes PyMySQL); **E-31** (`/health` getrennt von Datenaktualität); DTB-13 (Stale/Plausibilität), DTB-43 (503-Pfad), DTB-64 (Assess-/Serve-Zeit-Verdrahtung); blockt DTB-20 (P3.2). Querverweis persönliches Log: DTB-64-Eintrag (2026-06-26).
