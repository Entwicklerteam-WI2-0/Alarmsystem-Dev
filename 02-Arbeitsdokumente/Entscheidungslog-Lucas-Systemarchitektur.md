...
**E-41 — Sensor-Defekt-Erkennung im Ingest-Poller (DTB-20, P3.2): Flatline über Zeitfenster + Sprung + Timeout → `unknown`**
- *Status:* Umgesetzt (PR #128, 2026-06-28); Code-Review ohne CRITICAL/HIGH. Implementiert Schicht „Plausibilität" aus E-40.
...
- *Offen (vor M3 zu schließen):* **(i)** `epsilon` ist an DS18B20-12-Bit gebunden — **G1/Nils muss Bit-Tiefe (9–12) + ob gemittelt bestätigen**; bei < 12-Bit skaliert `epsilon` (0,15/0,30/0,50/1,0). Bleibt parametrierbar in `config/thresholds.json` (NF-05, kein Hardcode). **(ii) Flatline-Recovery-Tradeoff (K1, Architektenentscheidung ausstehend):** nach erkannter Flatline wird das Fenster bewusst **nicht** zurückgesetzt (sonst akzeptierte ein stuck-Sensor ~14/15 min wieder als gültig → NF-01-Regression); Recovery erst, wenn die Temperatur das Band > `epsilon` verlässt. Wie lange ein Sensor bei nur marginaler Bewegung gesperrt bleibt, ist zu dokumentieren.
